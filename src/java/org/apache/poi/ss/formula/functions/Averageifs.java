...
public final class Averageifs implements FreeRefFunction {
	...
	public ValueEval evaluate(ValueEval[] args, OperationEvaluationContext ec) {
		...
		try {
            AreaEval averageRange = convertRangeArg(args[0]);
            ...
            validateCriteriaRanges(ae, averageRange);

            double result = averageMatchingCells(ae, mp, averageRange);
     ...
     * as the <code>averageRange</code> argument
     *
     * @throws EvaluationException if
     */
    private void validateCriteriaRanges(AreaEval[] criteriaRanges, AreaEval averageRange) throws EvaluationException {
        for(AreaEval r : criteriaRanges){
            if(r.getHeight() != averageRange.getHeight() ||
               r.getWidth() != averageRange.getWidth() ) {
    ...
    /**
     *
     * @param ranges  criteria ranges, each range must be of the same dimensions as <code>aeAverage</code>
     * @param predicates  array of predicates, a predicate for each value in <code>ranges</code>
     * @param aeAverage  the range to average
     *
     * @return the computed value
     */
    private static double averageMatchingCells(AreaEval[] ranges, I_MatchPredicate[] predicates, AreaEval aeAverage) throws EvaluationException {
        int height = aeAverage.getHeight();
        int width = aeAverage.getWidth();
        ...
        for (int r = 0; r < height; r++) {
            for (int c = 0; c < width; c++) {
                ...
                for(int i = 0; i < ranges.length; i++){
                    ...
                    System.out.println(aeRange.getRelativeValue(r, c) + ", " + !mp.matches(aeRange.getRelativeValue(r, c)));
	...
	private static double accumulate(AreaEval aeAverage, int relRowIndex,
			int relColIndex) {
