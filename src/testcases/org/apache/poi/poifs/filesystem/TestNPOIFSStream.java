...
public final class TestNPOIFSStream extends TestCase {
   ...
   public void testReadMiniStreams() throws Exception {
      try (final NPOIFSFileSystem fs = new NPOIFSFileSystem(_inst.openResourceAsStream("BlockSize512.zvi"))) {
          final NPOIFSMiniStore ministore = fs.getMiniStore();
      
          // 178 -> 179 -> 180 -> end
          final NPOIFSStream stream = new NPOIFSStream(ministore, 178);
          final Iterator<ByteBuffer> i = stream.getBlockIterator();
          assertEquals(true, i.hasNext());
          assertEquals(true, i.hasNext());
          assertEquals(true, i.hasNext());
   ...
   public void testReplaceStreamWithMore() throws Exception {
      try (NPOIFSFileSystem fs = new NPOIFSFileSystem(_inst.openResourceAsStream("BlockSize512.zvi"))) {
   ...
   public void testWriteStream4096() throws Exception {
      try (final NPOIFSFileSystem fs = new NPOIFSFileSystem(_inst.openResourceAsStream("BlockSize4096.zvi"))) {
