...
public class CalculationChain extends POIXMLDocumentPart {
    ...
    public void removeItem(int sheetId, String ref){
        ...
        int id = -1;
        final List<CTCalcCell> l = chain.getCList();

        final Iterator<CTCalcCell> it = l.iterator();

        CTCalcCell next = null;
        if (it.hasNext()) {
            next = it.next();
        }
        int i = 0;
        do {
            ...
            if(c.isSetI()) id = c.getI();

            if(id == sheetId && c.getR().equals(ref)){
                if(c.isSetI() && next != null && !next.isSetI()) {
                    next.setI(id);
                }
                chain.removeC(i);
                break;
